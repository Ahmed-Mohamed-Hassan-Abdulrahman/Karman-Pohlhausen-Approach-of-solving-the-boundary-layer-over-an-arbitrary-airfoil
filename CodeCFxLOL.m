...
del1=[-120^-1 3/10];
del2=[-1/9072 -1/945 37/315];
k_p=conv(conv(del2,del2),[1 0]);
f2_p=conv([1/6 2],del2);
F_del2=(conv((2*[0 0 f2_p]-4*k_p),del2)-[0 conv(2*k_p,del1)]);
lamda_s=roots(F_del2);
a=real(lamda_s)<10 & real(lamda_s)>-12;
...
for i = d+1:length(U_d)
    ...
    Re_delta2_(i-1) = U(i-1) .* delta2(i-1) * (rho/mu);
